...
Start the training of the model using the following command. The training logs and model breakpoints (`.pkl`files) will be saved in the `output/` directory.
...
Use the following command to evaluate the breakpoints of a trained model. Please replace the path after the `--resume` parameter with the model file you want to test.
```bash
...
By comparison, my Jittor implementation version achieved the highest validation set accuracy of **62%** on the `cats_vs_dogs` dataset, which is very close in numerical terms to the benchmark result of **66.0%** achieved by PyTorch on smaller datasets.
As can be seen from the alignment curve graph above, **the Jittor accuracy curve fluctuates within a similar range to that of PyTorch**. Due to the small dataset, both reach their performance peaks in the first five epochs and then experience a slight decline or stagnation in performance in the later stage due to overfitting on the small dataset.
